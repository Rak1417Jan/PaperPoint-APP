...
class placeorderListcompleted extends StatefulWidget {
  const placeorderListcompleted({Key? key}) : super(key: key);

  @override
  State<placeorderListcompleted> createState() => _placeorderListStatecompleted();
}

class _placeorderListStatecompleted extends State<placeorderListcompleted> {
          ...
          child: Column(
            ...
            children: <Widget>[
              Row(
                children: [
                  SizedBox(
                      width: screenWidth*0.375, child: _buildHeader("Completed", "Orders")),
                  Column(
                    children: [
                      // Image.asset(
                      ...
                      // ),
                      Lottie.asset('assets/completedOrders.json',width: screenWidth*0.5,repeat: true),
                      SizedBox(height: screenheight*0.0032),
                      Text("Admin",style: TextStyle(fontSize: screenWidth*0.054,color: Colors.grey, fontWeight: FontWeight.bold),)
                    ],
                  ),
                    ...
                    children: [
                      ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          minimumSize:
                          Size(screenWidth*0.4, screenheight*0.08), // Set the width and height
                        ),
                        child: Text(
                          "Refresh",
                          style: TextStyle(
  ...
  Future<List<PlaceOrderItem>> getOrdersForUser() async {
    CollectionReference orders = FirebaseFirestore.instance.collection(
        'Orders');
    List<String> dispatchstatusList = [
      'Not Yet Dispatched',
      'Paper type not available',
      'Paper Size not available',
      'Paper GSM not available',
      'Quantity not available',
      'Order Cancelled'
    ];
    List<String> deliverstatusList = [
      'Not Yet Delivered',
      'Paper type not available',
      'Paper Size not available',
      'Paper GSM not available',
    ...
    QuerySnapshot querySnapshot = await orders.where('dispatchTime', whereNotIn: dispatchstatusList).
    orderBy('dispatchTime').orderBy('orderDate').
    get();
